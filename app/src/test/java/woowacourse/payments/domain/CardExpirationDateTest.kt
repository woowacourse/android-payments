...
class CardExpirationDateTest {
    ...
    fun `과거 날짜일 경우 만료되었다`() {
        // given
        val dateString = "1220"
        val pastDate = CardExpirationDate.from(dateString, formatter)
    ...
    fun `미래 날짜일 경우 만료되지 않았다`() {
        // given
        val dateString = "0130"
        val futureDate = CardExpirationDate.from(dateString, formatter)
    ...
    fun `현재 날짜와 같을 경우 만료되지 않았다`() {
        // given
        val dateString = "0925"
        val currentDate = CardExpirationDate.from(dateString, formatter)
