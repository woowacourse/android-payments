...
class CardTest {
    ...
    fun `올바른 값으로 카드를 생성하면 성공한다`() {
        ...
        // when
        val card = Card.newCard(validNumber, validExpiration, validName, validPassword)
    ...
    fun `잘못된 카드 번호로 카드를 생성하면 예외 발생`() {
        val exception =
            assertThrows<IllegalArgumentException> {
                Card.newCard("1234", "12/99", "TAMA SEO", "1234")
            }
    ...
    fun `만료된 카드로 생성하면 예외 발생`() {
        ...
        val exception =
            assertThrows<IllegalArgumentException> {
                Card.newCard("1234567890123456", expired, "TAMA SEO", "1234")
            }
    ...
    fun `잘못된 카드 소지자 이름으로 생성하면 예외 발생`() {
        val exception =
            assertThrows<IllegalArgumentException> {
                Card.newCard("1234567890123456", "12/99", "TAMA_SEO", "1234")
            }
    ...
    fun `잘못된 비밀번호로 생성하면 예외 발생`() {
        val exception =
            assertThrows<IllegalArgumentException> {
                Card.newCard("1234567890123456", "12/99", "TAMA SEO", "12a4")
            }
