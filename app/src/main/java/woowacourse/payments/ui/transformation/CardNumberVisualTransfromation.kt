...
class CardNumberVisualTransformation : VisualTransformation {
    override fun filter(text: AnnotatedString): TransformedText {
        ...
        val trimmedOriginal =
            if (originalText.length >= CARD_NUMBER_MAX_LENGTH) {
        ...
        val numberOffsetTranslator =
            object : OffsetMapping {
                override fun originalToTransformed(offset: Int): Int {
                    val separatorsAdded =
                        (offset / CARD_NUMBER_GROUP_SIZE) * CARD_NUMBER_SEPARATOR.length
                ...
                override fun transformedToOriginal(offset: Int): Int {
                    val separatorsCount =
                        (offset / (CARD_NUMBER_GROUP_SIZE + CARD_NUMBER_SEPARATOR.length)) * CARD_NUMBER_SEPARATOR.length
