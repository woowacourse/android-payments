...
class CardUiStateHolderTest {
    ...
    fun `EMPTY_상태에서_카드가_추가되면_SINGLE_상태로_전환된다`() {
        // given
        val holder = CardUiStateHolder()
        val card = Card("1111", "12/34", "홍길동", "12", BankType.Bank(Banks.BC)).toSerializationCard()
    ...
    fun `SINGLE_상태에서_카드가_추가되면_MULTIPLE_상태로_전환되고_toolbarActionButtonVisibility_상태가_true가된다`() {
        // given
        val firstCard = Card("1111", "12/34", "페토", "1234", BankType.Bank(Banks.BC)).toSerializationCard()
        val secondCard = Card("2222", "56/78", "정페토", "1234", BankType.Bank(Banks.BC)).toSerializationCard()
        val holder = CardUiStateHolder()
    ...
    fun `Saver로_SINGLE_상태를_저장하고_복원한다`() {
        // given
        val card = Card("1111", "12/34", "페토", "1234", BankType.Bank(Banks.BC))
    ...
    fun `Saver로_MULTIPLE_상태를_저장하고_복원한다`() {
        // given
        val card1 = Card("1111", "12/34", "페토", "1234", BankType.Bank(Banks.BC))
        val card2 = Card("2222", "56/78", "정페토", "1234", BankType.Bank(Banks.BC))
